...
class Generator (analyser : Analyser) extends PrettyPrinter {
    ...
    def generateSupportFile (flags : Flags, supportFile : File) {
        ...
        val parseFailureMessagesContents =
            """
            |    import org.kiama.util.Messaging.{message, Messages}
            |    import org.kiama.util.Positions.{setFinish, setStart}
            |    import xtc.parser.{ParserBase, ParseError}
            |
            |    def ratsFailureMessages (p : ParserBase, error : ParseError) : Messages = {
            |        val loc = p.location (error.index)
            |        val pos = new LineColPosition (p, error.index, loc.line, loc.column)
            |        setStart (error, pos)
            |        setFinish (error, pos)
        ...
        val contents = s"""
            ...
            |
            |package sbtrats
            |${if (flags.useScalaPositions) lineColPositionContents else ""}
            |trait Action[T] {
            |    def run (arg : T) : T
            |}
            |
            ...
            |        result
            |    }
            |${if (flags.useScalaPositions && (flags.useKiama == 2)) parseFailureMessagesContents else ""}
            |}
            |
            |""".stripMargin
